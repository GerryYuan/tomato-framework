...
public class MqttProperties {
    ...
    private int qos = 1;
    /**
     * #若心跳时间过短，在消费端繁忙时会无法完成服务端的心跳回复导致服务端认为消费端下线而无法完成订阅 #可采用MQ接收端和消费端之间加上队列
     */
    ...
    public String getSubscribeId() {
        if (StringUtils.isEmpty(subscribeId)) {
            try {
                return "sub@" + InetAddress.getLocalHost().getHostAddress();
    ...
    public String getPublisherId() {
        if (StringUtils.isEmpty(publisherId)) {
            try {
                return "pub@" + InetAddress.getLocalHost().getHostAddress();
            } catch (UnknownHostException e) {
                throw new MqttException(e);
            }
        }
